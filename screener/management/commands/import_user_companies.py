...
from screener.models import (
    ...
    ShareholdingPattern, ManagementReliabilityScore, ManagementForecast, ForecastComparison
)

# Raw CSV Data provided by the user (500 Companies)
RAW_CSV_DATA = """Company Name,Industry,Symbol,Series,ISIN Code
...
Aster DM Quality Care Ltd.,Healthcare,ASTERDM,EQ,INE914M01019
...
Prime Focus Ltd.,Media Entertainment & Publication,PFOCUS,EQ,INE367G01038
...
class Command(BaseCommand):
    help = 'Imports all 500 companies provided by user into the screener database with detailed metrics & management reliability scores.'

    def handle(self, *args, **options):
        lines = RAW_CSV_DATA.strip().split('\n')[1:]
        self.stdout.write(f"Importing {len(lines)} companies into screener database...")

        imported_count = 0
        with transaction.atomic(using='screener'):
            for line in lines:
                ...
                sector = industry
                if industry in ['Information Technology']:
                    sector = 'Technology'
                elif industry in ['Financial Services']:
                    sector = 'Banking & Financials'
                elif industry in ['Healthcare']:
                    sector = 'Healthcare & Pharma'
                elif industry in ['Fast Moving Consumer Goods', 'Consumer Durables', 'Consumer Services']:
                    sector = 'Consumer & Retail'
                elif industry in ['Automobile and Auto Components']:
                    sector = 'Automotive'
                elif industry in ['Capital Goods', 'Construction Materials', 'Construction']:
                    sector = 'Industrials & Infrastructure'

                # Seed pseudo-random reproducible metrics per symbol hash
                seed_val = sum(ord(c) for c in symbol)
                rnd = random.Random(seed_val)

                cur_price = round(rnd.uniform(50, 4500), 2)
                mcap = round(rnd.uniform(2000, 1500000), 2)
                pe = round(rnd.uniform(8, 75), 2)
                pb = round(rnd.uniform(1.2, 12), 2)
                roe_val = round(rnd.uniform(8, 48), 2)
                roce_val = round(rnd.uniform(10, 55), 2)
                debt_val = round(rnd.uniform(0, mcap * 0.35), 2)
                sales_g = round(rnd.uniform(5, 35), 2)
                profit_g = round(rnd.uniform(8, 42), 2)
                div_y = round(rnd.uniform(0.2, 4.5), 2)
                prom_h = round(rnd.uniform(45, 75), 2)
                rel_score = rnd.randint(65, 96)
                foliux_sc = max(60, min(98, int((roe_val + roce_val + rel_score) / 3)))
                ...
                company, created = Company.objects.update_or_create(
                    ...
                    defaults={
                        ...
                        'industry': industry,
                        'sector': sector,
                        'about': f"{name} is a leading player in the {industry} industry operating in India.",
                        'current_price': cur_price,
                        'market_cap': mcap,
                        'high_52w': round(cur_price * 1.18, 2),
                        'low_52w': round(cur_price * 0.78, 2),
                        'pe_ratio': pe,
                        'pb_ratio': pb,
                        'book_value': round(cur_price / pb, 2),
                        'eps': round(cur_price / pe, 2),
                        'roe': roe_val,
                        'roce': roce_val,
                        'dividend_yield': div_y,
                        'debt': debt_val,
                        'cash': round(debt_val * 0.6, 2),
                        'intrinsic_value': round(cur_price * 1.15, 2),
                        'industry_pe': round(pe * 0.9, 2),
                        'promoter_holding': prom_h,
                        'sales_growth': sales_g,
                        'profit_growth': profit_g,
                        'foliux_score': foliux_sc,
                        'buy_score': rnd.randint(70, 95),
                        'valuation_score': rnd.randint(60, 90),
                        'growth_score': rnd.randint(65, 95),
                        'health_score': rnd.randint(70, 95),
                        'risk_score': rnd.randint(10, 30),
                        'technical_score': rnd.randint(65, 90),
                    }
                )

                # Seed ManagementReliabilityScore
                total_fc = rnd.randint(6, 16)
                achieved_fc = int(total_fc * (rel_score / 100.0))
                delayed_fc = rnd.randint(0, 2)
                not_achieved_fc = max(0, total_fc - achieved_fc - delayed_fc)
                partially_fc = total_fc - achieved_fc - delayed_fc - not_achieved_fc

                ManagementReliabilityScore.objects.update_or_create(
                    company=company,
                    defaults={
                        'total_forecasts': total_fc,
                        'achieved_count': achieved_fc,
                        'partially_achieved_count': max(0, partially_fc),
                        'not_achieved_count': not_achieved_fc,
                        'delayed_count': delayed_fc,
                        'reliability_score_percent': float(rel_score),
                        'ai_credibility_rating': 'High' if rel_score >= 80 else 'Moderate',
                        'overall_score': rel_score,
                    }
                )

                # Seed Quarterly & Annual Profit/Loss baseline metrics
                for q_name in ['Q1 FY25', 'Q2 FY25', 'Q3 FY25', 'Q4 FY25']:
                    QuarterlyResult.objects.get_or_create(
                        company=company,
                        quarter=q_name,
                        defaults={
                            'sales': round(mcap * 0.05, 2),
                            'operating_profit': round(mcap * 0.012, 2),
                            'opm_percent': round(rnd.uniform(12, 30), 2),
                            'other_income': round(mcap * 0.001, 2),
                            'interest': round(mcap * 0.001, 2),
                            'depreciation': round(mcap * 0.002, 2),
                            'profit_before_tax': round(mcap * 0.008, 2),
                            'tax_percent': 25.0,
                            'net_profit': round(mcap * 0.006, 2),
                            'eps': round(cur_price / pe / 4, 2),
                        }
                    )

                imported_count += 1

        self.stdout.write(self.style.SUCCESS(f"Successfully imported and seeded {imported_count} companies!"))
